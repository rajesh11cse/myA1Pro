...
var Platform = mongoose.model('Platform');                   // Schema Platform

require('../models/questionnaire.js');
var Questionnaire = mongoose.model('Questionnaire');         // Schema Questionnaire

require('../models/tokenBlacklist.js');
var TokenBlacklist = mongoose.model('TokenBlacklist'),       // Schema TokenBlackList
 ...
 */
function sendMailToUser(user, req) {
  return new Promise(function (resolve, reject) {
    ...
    var url = req.protocol + '://' + req.get('host') + '/#/auth/resetPassword?token='
    ...
    let mailOptions = {
      ...
      '<p><a href="' + url + token + '">Click here</a> to reset your password</p>' +
      "<p>Regards,</p>" +
      "<p>Your GigUp Team</p>"
    };
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      resolve('Password reset instructions have been e-mailed to you.');
    });
  });
};
...
module.exports.userRegistration = function (req, res) {
  ...
  User.findOne({ email: req.body.email }, function (err, user) {
    if (err) {
      log.error({ status: 200, success: false, message: 'Server error.' });
      return res.status(200).json({ success: false, message: 'Server error.' });
    }
    if (user) {
      log.error({ status: 200, success: false, message: 'Email address already exists !' });
      ...
      newUser.save(function (err, data) {
        if (err) {
          ...
          if (err.name == 'ValidationError') {
            log.error({ status: 200, success: false, message: err.message });
...
module.exports.updateUser = function (req, res) {
  ...
  jwt.verify(token.substring(4), configAuth.secret, function (err, decoded) {
    ...
    if (err) {
      log.error({ status: 200, success: false, message: 'Token is invalid or expired.' });
      return res.status(200).json({ success: false, message: 'Token is invalid or expired.' });
    }
    // update query
    var query = {
      $set: {
    ...
    if (err) {
      ...
      User.findOne({ _id: decoded.id }, function (err, user) {
        ...
        if (!user) {
          ...
          log.error({ status: 200, success: false, message: 'Authentication failed. User not found.' });
...
module.exports.checkEmailExistance = function (req, res) {
  User.findOne({ email: req.body.email }, function (err, user) {
    ...
    if (!user) {
      //  If no user found in database then revert back with error message
      log.error({ status: 200, success: false, message: 'Email address does not exist !' });
      res.status(200).json({ success: false, message: 'Email address does not exist !' });
      return;
    } else {
      // create the user token
      var token = jwt.sign({ id: user._id, email: user.email }, configAuth.secret, {
    ...
    if (err) {
      ...
      User.findOne({
        ...
        if (!user) {
          //  If no user found in database then revert back with error message
          log.error({ status: 200, success: false, message: 'Authentication failed. User not found.' });
          return res.status(200).json({ success: false, message: 'Authentication failed. User not found.' });
        }
        if (!user.validPassword(req.body.oldPassword)) {
          log.error({ status: 200, success: false, message: 'Current password is incorrect !' });
...
module.exports.forgotPassword = function (req, res, next) {
  findUserByEnmailId(req)
    .then(function (userData) {
      return sendMailToUser(userData, req);
    })
    .then(function (data) {
      return res.status(200).json({ success: true, message: data });
    })
    .catch(function (err) {
      log.error({ status: 200, success: false, message: err });
...
module.exports.updateUserMatchedPlatforms = function (req, res) {
  ...
  var query = {
    $set: {
      ...
      totalPFs: req.body.totalPFs
    }
  }
  // find the user using email address.
  User.findOne({ email: req.body.email }, function (err, user) {
    if (err || !user) {
      log.error({ status: 200, success: false, message: 'Invalid user !' });
  ...
  jwt.verify(token.substring(4), configAuth.secret, function (err, decoded) {
    ...
    if (err) {
      ...
      User.findOne({ _id: decoded.id }, function (err, user) {
        ...
        Platform.find({ id: { "$in": user.totalPFs } }, function (err, data) {
          if (err || !data) {
            log.error({ status: 200, success: false, message: 'No platform found.' });
            return res.status(200).json({ success: false, message: 'No platform found.' });
          }
          var platformObject = [];
          async.eachSeries(data, function (platform, callback) {

            var newPlatform = platform.toObject();
            newPlatform['isInterested'] = false;

            var matchedPFs = user.matchedPFs

            matchedPFs.forEach(function (val, index) {
              if (val == platform.id) {
                newPlatform.isInterested = true;
              }
            });

            platformObject.push(newPlatform);
            callback();
          }, function (err, result) {
            ...
            res.status(200).json({ success: true, data: platformObject });
          });
    ...
    if (err) {
      ...
      User.findOne({ _id: decoded.id }, function (err, user) {
        ...
        Questionnaire.find(query).exec(function (err, qnrs) {
          if (err || !qnrs) {
            log.error({ status: 200, success: false, message: 'No questionnaire found.' });
            return res.status(200).json({ success: false, message: 'No questionnaire found.' });
          }
          var obj = [];
